...
public class NewsIntentService extends HTTPIntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		try {
			...
			if (!cache.exists("newsItemList") || force) {
				String xml = cache.exists("newsItemList") ? fetch(HYDRA_BASE_URL
				    + "versions.xml") : fetch(HYDRA_BASE_URL + "versions.xml",
				    cache.lastModified("newsItemList"));
				list = getNewsItems(xml);
				cache.put("newsItemList", list);
			} else {
				list = cache.get("newsItemList");
			}
